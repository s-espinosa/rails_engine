...
RSpec.describe "GET /api/v1/items/find_all?parameters=:parameters" do
  it "returns all items with a name" do
    ...
    item1 = Item.create(name: "Item", description: "Item1 description", unit_price: 450, merchant_id: merchant.id)
    item2 = Item.create(name: "Item", description: "Item2 description", unit_price: 450, merchant_id: merchant.id)
  ...
  it "returns all items with a description" do
    merchant = Merchant.create(name: "Merchant")
    item1 = Item.create(name: "Item1", description: "Item description", unit_price: 450, merchant_id: merchant.id)
    item2 = Item.create(name: "Item2", description: "Item description", unit_price: 450, merchant_id: merchant.id)
  ...
  xit "returns all items with a unit_price" do
    merchant = Merchant.create(name: "Merchant")
    ...
    first_item = json_body[0]
    expect(first_item["id"]).to eq(item1[:id])
    expect(first_item["unit_price"]).to eq(4.50)
  end
  ...
  xit "returns all items with a merchant_id" do
    ...
    first_item = json_body[0]
    expect(first_item["id"]).to eq(item1[:id])
    expect(first_item["merchant_id"]).to eq("Item")
  end
end
